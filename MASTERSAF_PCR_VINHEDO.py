...
for dia in i:
    ...
    driver.find_element_by_xpath("//div[@data-value='b2b91a5241559d4601415b4e394e0000']").click()
    time.sleep(5)
    ...
    df.to_excel(f'//Qsbrprd/qliksense/BRASIL/OUTRAS FONTES/PCR_INDUSTRIAL/Finance/CONCILIAÇÃO SIR/Automatização/PCR_VINHEDO_MASTERSAF/{vardata}_RECEBIMENTO_NFE.xlsx', index = False)
    
    """
    CARREGANDO A BASE TOTAL - PCR VINHEDO
    """
    dfbase = pd.read_excel(r"\\Qsbrprd\qliksense\BRASIL\OUTRAS FONTES\PCR_INDUSTRIAL\Finance\CONCILIAÇÃO SIR\Automatização\PCR_VINHEDO_MASTERSAF\BD\BD_PCR_VINHEDO.xlsx")
    
    """
    CARREGANDO A CARGA INCREMENTAL
    """
    dfcarga = pd.read_excel(fr"\\Qsbrprd\qliksense\BRASIL\OUTRAS FONTES\PCR_INDUSTRIAL\Finance\CONCILIAÇÃO SIR\Automatização\PCR_VINHEDO_MASTERSAF\{vardata}_RECEBIMENTO_NFE.xlsx")
    ...
    dffinal.to_excel(r"\\Qsbrprd\qliksense\BRASIL\OUTRAS FONTES\PCR_INDUSTRIAL\Finance\CONCILIAÇÃO SIR\Automatização\PCR_VINHEDO_MASTERSAF\BD\BD_PCR_VINHEDO.xlsx", index = False)

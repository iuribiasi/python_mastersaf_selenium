...
for dia in i:
    ...
    driver.find_element_by_xpath("//div[@data-value='977da1114454c36a014454c499850001']").click()
    time.sleep(5)
    ...
    df.to_excel(f'//Qsbrprd/qliksense/BRASIL/OUTRAS FONTES/PCR_INDUSTRIAL/Finance/CONCILIAÇÃO SIR/Automatização/PPL_SUMARE_MASTERSAF/{vardata}_RECEBIMENTO_NFE.xlsx', index = False)
    
    """
    CARREGANDO A BASE TOTAL - PPL SUMARE
    """
    dfbase = pd.read_excel(r"\\Qsbrprd\qliksense\BRASIL\OUTRAS FONTES\PCR_INDUSTRIAL\Finance\CONCILIAÇÃO SIR\Automatização\PPL_SUMARE_MASTERSAF\BD\BD_PPL_SUMARE.xlsx")
    
    """
    CARREGANDO A CARGA INCREMENTAL
    """
    dfcarga = pd.read_excel(fr"\\Qsbrprd\qliksense\BRASIL\OUTRAS FONTES\PCR_INDUSTRIAL\Finance\CONCILIAÇÃO SIR\Automatização\PPL_SUMARE_MASTERSAF\{vardata}_RECEBIMENTO_NFE.xlsx")
    ...
    dffinal.to_excel(r"\\Qsbrprd\qliksense\BRASIL\OUTRAS FONTES\PCR_INDUSTRIAL\Finance\CONCILIAÇÃO SIR\Automatização\PPL_SUMARE_MASTERSAF\BD\BD_PPL_SUMARE.xlsx", index = False)

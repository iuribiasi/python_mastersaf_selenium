...
for dia in i:
    ...
    driver.find_element_by_xpath("//div[@data-value='8a5244705e45dbc1015e72238ae50001']").click()
    time.sleep(5)
    ...
    df.to_excel(f'//Qsbrprd/qliksense/BRASIL/OUTRAS FONTES/PCR_INDUSTRIAL/Finance/CONCILIAÇÃO SIR/Automatização/PPL_VINHEDO_MASTERSAF/{vardata}_RECEBIMENTO_NFE.xlsx', index = False)
    
    """
    CARREGANDO A BASE TOTAL - PPL VINHEDO
    """
    dfbase = pd.read_excel(r"\\Qsbrprd\qliksense\BRASIL\OUTRAS FONTES\PCR_INDUSTRIAL\Finance\CONCILIAÇÃO SIR\Automatização\PPL_VINHEDO_MASTERSAF\BD\BD_PPL_VINHEDO.xlsx")
    
    """
    CARREGANDO A CARGA INCREMENTAL
    """
    dfcarga = pd.read_excel(fr"\\Qsbrprd\qliksense\BRASIL\OUTRAS FONTES\PCR_INDUSTRIAL\Finance\CONCILIAÇÃO SIR\Automatização\PPL_VINHEDO_MASTERSAF\{vardata}_RECEBIMENTO_NFE.xlsx")
    ...
    dffinal.to_excel(r"\\Qsbrprd\qliksense\BRASIL\OUTRAS FONTES\PCR_INDUSTRIAL\Finance\CONCILIAÇÃO SIR\Automatização\PPL_VINHEDO_MASTERSAF\BD\BD_PPL_VINHEDO.xlsx", index = False)
